...
class LiveWriter:
    ...
    def write_detection(self, det: Detection) -> None:
        self._write_line("detection", det.model_dump(exclude_none=True))

    def write_annotation(self, profile: str, ann: Annotation) -> None:
        self._write_line("annotation", ann.model_dump(exclude_none=True), profile=profile)

    def write_meta(self, meta: Meta) -> None:
        self._write_line("meta", meta.model_dump(exclude_none=True))
...
class DailyAlgoWriter:
    ...
    def write_detection(self, det: Detection) -> None:
        ...
        self._write_line("detection", det.model_dump(exclude_none=True), ts_iso, str(det.event_id), None)

    def write_annotation(self, profile: str, ann: Annotation, event_id: str) -> None:
        ts_iso = ann.timestamp
        date_str = self._date_from_iso(ts_iso)
        self._ensure_handle(date_str)
        self._write_line("annotation", ann.model_dump(exclude_none=True), ts_iso, event_id, profile)

    def write_meta(self, meta: Meta) -> None:
        ts_iso = meta.started_at or meta.finished_at or to_iso_utc_z("1970-01-01T00:00:00Z")
        date_str = self._date_from_iso(ts_iso)
        self._ensure_handle(date_str)
        self._write_line("meta", meta.model_dump(exclude_none=True), ts_iso, "", None)
