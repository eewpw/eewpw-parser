...
class EpicShakeAlertDialect:
    PROFILE_NAME: str = "profiles/epic_time_vs_mag.json"
    ...
    def normalize_line(line: str) -> Optional[str]:
        m = EpicShakeAlertDialect.P_LEVEL.match(line)
        if not m:
            return None
    ...
    def _update_wall_clock(self, line: str, state: EpicStreamState) -> Optional[str]:
    ...
    def _parse_obs(self, obs_elem: ET.Element) -> GMObs:
        ...
        other_attrs = {
            k: v for k, v in obs_elem.attrib.items() if k not in {"orig_sys", "assoc"}
        }
        if other_attrs:
            obs_extra["attrs"] = other_attrs
        if field_units:
    ...
    def _parse_event_message(self, xml_text: str) -> Detection:
        ...
        event_message_extra = dict(attrs)
        ...
        if event_message_extra:
            det_extra.update(event_message_extra)
        if core_info_extra:
    ...
    def parse_stream(
        ...
        finalize: bool = False,
    ) -> Tuple[List[Detection], List[Annotation], EpicStreamState]:
        if state is None:
            state = EpicStreamState()
                    ...
                    if re.search(pat, line):
                        annotations.append(
                            Annotation(
                                ...
                                text=line.rstrip("\n"),
                                pattern_id=f"epic/{self.DIALECT_ID}:{idx}",
                            )
                        )
