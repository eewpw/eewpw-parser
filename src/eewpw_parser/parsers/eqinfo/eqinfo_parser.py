...
from eewpw_parser.parsers.eqinfo.dialects import EqinfoShakeAlertDialect, EqinfoStreamState
...
class EqinfoParser:
    def __init__(self, cfg: Dict[str, Any]):
        ...
        if requested and requested != "shakealert":
            raise ValueError("EqInfo parser supports only the shakealert dialect.")
    ...
    def parse(self, inputs: List[str], sink: Optional[BaseSink] = None) -> Optional[FinalDoc]:
        ...
        worker = EqinfoShakeAlertDialect()
    ...
    def _get_stream_worker(self) -> EqinfoShakeAlertDialect:
        if hasattr(self, "_stream_worker"):
            return self._stream_worker
        self._stream_worker = EqinfoShakeAlertDialect()
        return self._stream_worker
