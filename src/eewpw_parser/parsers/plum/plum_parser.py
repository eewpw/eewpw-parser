...
from eewpw_parser.parsers.plum.dialects import PlumDialect, PlumStreamState
...
class PlumParser:
    def __init__(self, cfg: Dict[str, Any]):
        self.cfg = cfg
        self.dialect = cfg.get("dialect", "plum")
        self.verbose = bool(cfg.get("verbose", False))

    def _expand_inputs(self, inputs: List[str]) -> List[str]:
        files: List[str] = []
        for p in inputs:
            path = Path(p)
            if path.is_dir():
                files.extend([str(x) for x in sorted(path.glob("*.xml"))])
            else:
                files.append(str(path))
        return files

    def parse(self, inputs: List[str], sink: Optional[BaseSink] = None) -> Optional[FinalDoc]:
        files = self._expand_inputs(inputs)
        dets_all = []
        ann_all = []
    ...
    def _get_stream_worker(self) -> PlumDialect:
        if hasattr(self, "_stream_worker"):
            return self._stream_worker
        self._stream_worker = PlumDialect()
        return self._stream_worker
