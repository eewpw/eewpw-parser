...
from eewpw_parser.parsers.gfast.dialects import GfastShakeAlertDialect, GfastStreamState
...
class GfastParser:
    def __init__(self, cfg: Dict[str, Any]):
        ...
        if requested and requested != "shakealert":
            raise ValueError("GFAST parser supports only the shakealert dialect.")
    ...
    def _get_stream_worker(self) -> GfastShakeAlertDialect:
        if hasattr(self, "_stream_worker"):
            return self._stream_worker
        self._stream_worker = GfastShakeAlertDialect()
        return self._stream_worker
