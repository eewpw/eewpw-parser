...
- Event parsing logic:
  - `feed_line` normalizes the prefix timestamp, records annotations through the shared annotation config flow (preferred `annotations.json`, with `profiles/vs_time_vs_mag.json` as deprecated fallback), and updates file start/end times.
