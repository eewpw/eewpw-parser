...
- Finder annotation patterns are loaded through the shared config flow. Preferred customization is `annotations.json` (via `--config-root`); legacy `profiles/*.json` files are fallback compatibility config and are deprecated/planned for removal. `SCFinderDialect` maps to `profiles/scfinder_time_vs_mag.json` as fallback, while `NativeFinderDialect`, `NativeFinderLegacyDialect`, and `ShakeAlertFinderDialect` map to `profiles/finder_time_vs_mag.json`. `_parse_annotations` walks the log, normalizes timestamps from the prefix, and records matches.
