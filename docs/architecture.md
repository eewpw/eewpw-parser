...
- `config.py` loads `global.json` and annotation config through `config_loader.py`. `annotations.json` is the preferred annotation source, with `profiles/*.json` kept as fallback compatibility. Runtime config root precedence is: CLI `--config-root` > `EEWPW_PARSER_CONFIG_ROOT` > packaged defaults under `src/eewpw_parser/configs/` (no repo-root fallback). Repo-root `./example-configs` is example-only.
...
- CLI supports `--mode batch` (default) to emit a single JSON file. `--mode stream-jsonl` remains available as a deprecated offline JSONL export and is not the same as live daily JSONL output. An optional `--instance` sets the instance id (default `<algo>@unknown`).
